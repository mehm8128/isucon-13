...
func reserveLivestreamHandler(c echo.Context) error {
	...
	if _, err := tx.NamedExecContext(ctx, "INSERT INTO livestream_tags (livestream_id, tag_id) VALUES (:livestream_id, :tag_id)", tagsToBeInserted); err != nil {
...
func searchLivestreamsHandler(c echo.Context) error {
	...
	if c.QueryParam("tag") != "" {
		...
		if err := tx.SelectContext(ctx, &keyTaggedLivestreams, `
			SELECT livestream_tags.* FROM livestream_tags
			INNER JOIN tags ON tags.id = livestream_tags.tag_id
			WHERE tags.name = ?
			ORDER BY livestream_tags.livestream_id DESC
		`, keyTagName); err != nil {
